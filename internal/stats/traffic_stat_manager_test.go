...
func TestTrafficStatManager_Add(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		manager.Add(1, 1, "google.com", 1, 0, 0, 0, 0, 0, 0, false, 0)
	}
...
func TestTrafficStatManager_Upload(t *testing.T) {
	manager := NewTrafficStatManager()
	for i := 0; i < 100; i++ {
		manager.Add(1, 1, "google.com"+types.String(rands.Int(0, 10)), 1, 0, 1, 0, 0, 0, 0, false, 0)
	}
	err := manager.Upload()
	if err != nil {
		t.Fatal(err)
...
func BenchmarkTrafficStatManager_Add(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			manager.Add(1, 1, "google.com"+types.String(rand.Int63()%10), 1024, 1, 0, 0, 0, 0, 0, false, 0)
		}
	})
}
